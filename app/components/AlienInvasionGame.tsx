    ...
    useEffect(() => {
        ...
        function createBullet(x: number, y: number, targetX: number, targetY: number): Bullet {
            ...
            return {
                ...
                vx: (distance > 0.0001 ? dx / distance : 0) * 7,
                vy: (distance > 0.0001 ? dy / distance : -1) * 7
            };
        }
        ...
        function updateBullets() {
            ...
            for (let i = bulletsRef.current.length - 1; i >= 0; i--) {
                ...
                bullet.x += bullet.vx * speedMultiplier.current;
                bullet.y += bullet.vy * speedMultiplier.current;

                if (!Number.isFinite(bullet.x) || !Number.isFinite(bullet.y) ||
                    bullet.y < 0 || bullet.y > 700 || // Use fixed canvas height
        ...
        function drawPlayer(ctx: CanvasRenderingContext2D) {
            ...
            // Blue ninja mask when prepared (cover entire head with only eyes visible)
            if (gameRef.current.prepared) {
                // Full head covering (larger than head to clearly read as a hood)
                ctx.fillStyle = '#1E90FF';
                ctx.beginPath();
                ctx.arc(player.x + 15, player.y + 10, 10, 0, Math.PI * 2);
                ctx.fill();

                // Lower wrap/hood under the head
                ctx.fillStyle = '#1772CC';
                ctx.beginPath();
                ctx.moveTo(player.x + 5, player.y + 14);
                ctx.quadraticCurveTo(player.x + 15, player.y + 20, player.x + 25, player.y + 14);
                ctx.lineTo(player.x + 25, player.y + 19);
                ctx.quadraticCurveTo(player.x + 15, player.y + 25, player.x + 5, player.y + 19);
                ctx.closePath();
                ctx.fill();

                // Outline for definition
                ctx.strokeStyle = '#0F5FBF';
                ctx.lineWidth = 1;
                ctx.beginPath();
                ctx.arc(player.x + 15, player.y + 10, 10, 0, Math.PI * 2);
                ctx.stroke();
                ...
                // Knot and ties on the right side of the head
                ctx.fillStyle = '#1C7AD8';
                ctx.beginPath();
                ctx.arc(player.x + 27, player.y + 10, 2.5, 0, Math.PI * 2);
                ctx.fill();

                // Two ribbon tails
                ctx.beginPath();
                ctx.moveTo(player.x + 29, player.y + 10);
                ctx.lineTo(player.x + 36, player.y + 7);
                ctx.lineTo(player.x + 32, player.y + 12);
                ctx.closePath();
                ctx.fill();

                ctx.beginPath();
                ctx.moveTo(player.x + 29, player.y + 10);
                ctx.lineTo(player.x + 36, player.y + 13);
                ctx.lineTo(player.x + 32, player.y + 8);
                ctx.closePath();
                ctx.fill();

                // Eye openings and pupils on top of mask
                ctx.fillStyle = '#FFFFFF';
                ctx.beginPath();
                ctx.arc(player.x + 11, player.y + 8, 2.4, 0, Math.PI * 2);
                ctx.fill();
                ctx.beginPath();
                ctx.arc(player.x + 19, player.y + 8, 2.4, 0, Math.PI * 2);
                ctx.fill();

                ctx.fillStyle = '#000';
                ctx.beginPath();
                ctx.arc(player.x + 11, player.y + 8, 1.2, 0, Math.PI * 2);
                ctx.fill();
                ctx.beginPath();
                ctx.arc(player.x + 19, player.y + 8, 1.2, 0, Math.PI * 2);
                ctx.fill();
            }
